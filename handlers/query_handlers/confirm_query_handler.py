...
async def language_query_handler(callback_query: CallbackQuery, callback_data: Dict[str, str]):
    if str(callback_data["id"]) == str(callback_query.from_user.id):
        if callback_data["button"] != "Men odamman":
            ...
            await callback_query.message.answer(
                text=f"<a href='{callback_query.from_user.username}'>{callback_query.from_user.full_name}</a> "
                     f"safimizga xush kelibsiz! Gurux qoidalarini buzmang",
                disable_web_page_preview=True
            )
    else:
        await callback_query.answer(text="Bu tugma siz uchun emas", show_alert=True)